...
public class PATTree implements java.io.Serializable {
  ...
  public ArrayList getSuffixPerplexity(String word, int suffixLen) {
    ...
    if (tree != null) {
      ...
      Collections.sort(result, new SPComparator());
    }

    return result;
  }

  public class SPComparator implements Comparator {
    public int compare(Object o1, Object o2) {
      return (((SuffixPerplexity) o1).freq - ((SuffixPerplexity) o2).freq);
    }
  }
  ...
  public static void main(String[] args) {
    ...
    pattree.insert("人腦");
    pattree.print();
    PATTree invpattree = new PATTree();
    invpattree.insert(Strings.inverse("個人電腦"));
    invpattree.insert(Strings.inverse("人腦"));
    invpattree.print();
    String word = "腦";
    int suffixLen = 2;
    System.out.println("Frequency of the word " + word + " is " +
        pattree.getFrequency(word));
    ArrayList sps = pattree.getSuffixPerplexity(word, suffixLen);
    for (int i = 0; i < sps.size(); i++) {
      SuffixPerplexity sp = (SuffixPerplexity) sps.get(i);
      System.out.println("Suffix: " + sp.suffix + " = " + sp.freq);
    }
    ArrayList invsps = invpattree.getSuffixPerplexity(Strings.inverse(word),
        suffixLen);
    for (int i = 0; i < invsps.size(); i++) {
      SuffixPerplexity sp = (SuffixPerplexity) invsps.get(i);
      System.out.println("Prefix: " + sp.suffix + " = " + sp.freq);
    }
    /*
       PATTree pattree = new PATTree(Strings.EnglishLike);
       pattree.insert("This is a book");
       pattree.insert("One book");
       pattree.print();
       PATTree invpattree = new PATTree(Strings.EnglishLike);
       invpattree.insert(Strings.inverse("This is a book", Strings.EnglishLike));
       invpattree.insert(Strings.inverse("One book", Strings.EnglishLike));
       invpattree.print();
       String word = "a book";
       int suffixLen = 2;
       System.out.println("Frequency of the word " + word + " is " +
